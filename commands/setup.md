...
Install the `grab:` URI scheme handler so `[Grab 🫳](grab:...)` links put content in the clipboard.
...
5. On success, tell the user the `[Grab 🫳]` links will appear starting from the next session (the SessionStart hook detects the handler).
